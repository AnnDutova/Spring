...
public class TimeTableJSONParser
{
    ...
    public void writeStatistic(Statistic statistic)
    {
        ...
        {
            file.write(new Gson().toJson(statistic));
            file.flush();
        }
        catch(IOException e){
            e.printStackTrace();
    ...
    public ArrayList<DockedShip> readJSON(String timeTable)
    {
        Type listType = new TypeToken<ArrayList<DockedShip>>(){}.getType();
        ArrayList<DockedShip> tasker = new Gson().fromJson(timeTable, listType);
        tasker.sort(new DataComparator());
        return tasker;
    }

    public ArrayList<Task> readJSONTask(String timeTable)
    {
        ArrayList<DockedShip> readied = readJSON(timeTable);
        ArrayList<Task> tasker = new ArrayList<>();
        for (DockedShip ship: readied)
        {
